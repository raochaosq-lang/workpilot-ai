...
## v0.3.0 — 2026-07-04 — 部署就绪 + 产品卓越性 + 回归复扫三轮战役 (R11–R13)，公网部署更新

Tag: `v0.3.0` (SemVer minor — 首载性能/启动正确性/产品诚实与打磨/暗色与移动端补齐，无破坏性变更). 三轮 fleet 对抗式深扫（每轮 6 finder 并行 + 独立 skeptic 复核）+ 一次完整真实浏览器用户旅程走查（亮/暗/移动端），并把 21 个此前未推送的提交连同本轮成果发布到公网 GitHub Pages。

### R11 部署就绪 / 首载 / 启动韧性（25 确认 → 15 根因，全部修复）

- **CloudBase SDK 懒加载**：曾以解析阻塞方式无条件加载未固定版本的 `latest` SDK（~900KB）——纯本地用户也要付出整包下载，CDN 不可达时页面渲染完成但全部按钮死亡。改为 `ensureCloudbaseSdk()` 按需注入 + 固定 `2.31.0`（CDN 无 CORS 头，SRI 不可用，固定版本防 latest 静默变更），`getCloudBaseClient` 全链路 async 化。
- **启动身份水合**：`loadAll()` 曾先于会话恢复执行，已登录用户每次刷新都以匿名身份读草稿、随后 render→saveState 把匿名内容写进云端身份槽（数据丢失 + 跨身份泄漏）。新增 `hydratePersistedSession()` 先同步预读持久化身份；恢复失败降级时重载本地槽数据。
- 全局 error/unhandledrejection 兜底 + 启动失败可见横幅 + `bootstrapAuth` catch（不再有无声白屏）。
- 弹层焦点还原不再落到被遮挡的背景按钮（`getFallbackFocusTarget` 停留在最上层弹窗内）。
- 浏览器放不了 ≠ 服务端转不了：webm/ogg 在 Safari 上仅提示无法预览，服务端 ASR 转写保持可用（`isLikelyTranscribableAudio` 软硬分级）；SiliconFlow webm→wav 本地解码失败给出针对性出路。
- 页面外壳：meta description / og / theme-color / noscript；favicon 234KB→4.9KB（64px）+ 180px touch icon + 96px 头图；`[::1]` IPv6 本地源识别；copyText 临时节点 try/finally 清理；API 加载跑马灯在规则回退时立即停止；`SENLO_CONFIG_LOAD_ERROR` 有了消费者（诊断面板）；README 澄清 Pages 部署即纯本地模式、env.js 404 属正常；`.gitignore` 补 `.agents/`、`skills-lock.json`；README 移除本机路径泄漏。

### 旅程走查修复（浏览器实测发现，9 项）

- **规则引擎诚实性**：不再输出转写中不存在的模板顾虑（如对非 AI 岗位断言「AI 技术纵深不够」）；含具体数字（4.2s→1.6s）的回答不再被套话点评「缺少可验证结果」（`hasQuantifiedEvidence`）；候选人自认的知识盲区成为有据顾虑并进待提升；技术面不再因结尾一句「HR 会联系你聊薪资」被标成 HR 面（信号密度判定）；「说说/谈谈」式提问被识别；候选人反问不再被当考题打分。
- **已关联记录回填**：报告档案卡直接使用已关联面试记录的公司/岗位/轮次，不再显示「待识别」。
- **虚构时间戳**：无时间戳转写的合成 18s/行锚点不再以时钟形式展示（说话人视图隐藏时间列，来源按钮显示「第 N 段」）。
- 暗色模式约 50 处硬编码亮色表面补齐；移动端管理页操作条 390px 拉伸修复（flex-basis 泄漏进列布局）。

### R12 产品卓越性（27 原始 → 24 确认，23 修复，1 暂缓）

- **防误毁**：「填入示例」与历史「打开」都不再一键静默覆盖未保存的转写/结果（确认守护 + `isWorkspacePersistedInHistory`）；已自动写入历史的结果切换时不再误弹「未保存」危险确认。
- **诚实**：录音卡不再宣称「浏览器本地转写」（Web Speech 实为厂商服务器转写，改为「需联网」表述）；保存失败不再被成功 toast 掩盖（历史/模型配置/自动写入全部按真实结果分支提示）；规则引擎产物去掉「AI 点评/AI 超浓缩总结」署名（真实模型产物保留）；示例转写生成的报告在历史中带「示例 ·」前缀。
- **克制**：删除死重按钮「粘贴到下方」；「保存到历史」只在有未持久化内容时出现（编辑后显示「保存修改到历史」）；路由面板同屏重复展示去除；单份 Markdown 导出不再逐次弹隐私确认（完整 JSON 快照保留确认）；空工作区导出直接提示而非下载空文件。
- 视觉/文案：regenerate 强调态特异性修复；暗色状态药丸/危险按钮/选中文件卡/移动端吸底工具条补齐；「待提交」改为动词匹配的「待记录」；就绪态双按钮矛盾文案统一；中英夹排空格簇修正。
- 暂缓（产品决策）：「去复盘」的 link-and-keep 第三路径（需三选一弹窗）。

### R13 回归复扫（26 原始 → 25 确认，全部修复；含 3 条测试套件自身缺陷）

- **去复盘模板被路由劫持（high）**：填了 JD 的记录点「去复盘」后，模板头部的"岗位/JD"关键词把工作区识别成猎头沟通——主链路系统性走错输出类型。修复：识别到应用自产模板标记直接判定面试上下文 + 注入时跳过重检；顺带修复强猎头信号无视面试信号密度的问题（一句"HR说下周约终面"不再翻转整篇复盘笔记）。
- **测试连接静默持久化坏配置（high）**：部分测试成功就把整份表单（含改坏的 Key）写进存储并同步云端，"取消"形同虚设。修复：所有已配置目标全部通过才持久化；关闭弹窗恢复打开时的快照。
- 生成引擎：第二位面试官的追问不再被并进候选人回答打分；"稳定在60-80万年包"这类含话题词的真实薪资不再被占位词过滤清零（自相矛盾报告）；null/空串评分不再伪造"0分·需重练"；中英混合按比例判定；模型返回顶层数组不再误取内部对象；「我：/他：」自记体说话人被识别。
- 数据与同步：旧草稿迁移改为"先写新槽成功再删旧槽"；linkSummary/公司识别/取消关联在持久化失败时回滚并如实提示；历史去重的云端删除移到本地持久化成功之后（配额失败不再全端销毁旧记录）；音频元数据只在时长变化时持久化一次（不再每个按键重写全表 + 云端 upsert，也不再重置 createdAt）。
- ASR：真实转写路径补上 200-with-{error} 检查（配额/鉴权失败不再误报"转写接口返回为空"）；webm→wav 转换下混单声道 + 重采样 16kHz 并强制大小上限（12MB 长录音不再膨胀成数百 MB OOM）。
- 导出安全：Markdown 键名与 modelUsed 标签补转义（被篡改数据不能注入远程图片行）；文件名过滤只剔除不可见控制符，破折号/弯引号等可见排版符号保留。
- **测试套件自身**：两条冒烟守护因正则无界跨函数匹配而形同虚设（变异验证：删掉被守护的代码套件仍绿）——已收紧量词并补可执行断言；parseModelJson 畸形括号路径补上执行 fixture。
