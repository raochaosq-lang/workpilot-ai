...
未配置 CloudBase 时，普通用户会看到“本地体验”，不会承诺登录或多设备同步。CloudBase 配置完成后，普通用户的“登录 / 注册”弹窗不展示 CloudBase 技术配置。开发者本地调试时先用 `admin / datou123` 进入管理员模式，也可以访问：

```text
http://localhost:8001/?devCloud=1
```

进入管理员模式后，页面右上角会出现“开发者配置”，可填写：
...
3. 创建上述集合。
4. 配置安全规则。
5. 通过 `assets/env.js` 注入 CloudBase 配置；本地调试时可用 `admin / datou123` 进入管理员模式，并用 `?devCloud=1` 打开隐藏开发者配置。
...
- 配置并登录后可同步：面试记录、转写文本、AI 复盘报告、猎头/职业机会分析历史、上传文件 metadata、非密钥模型配置。
- `history_records` 和 `ai_summaries` 的报告 JSON 会保留 `contentType` 与 `recruiterSubtype`，用于区分面试复盘和猎头沟通整理；这不需要新增集合，也不改变现有安全规则原则。
